...
const columns = {
    ...
    inprogress: document.querySelector(
        '[data-col="inprogress"] .column__cards'
    ),
...
function refreshArrows(cardEl, columnName) {
    ...
    } else if (idx === columnOrder.length - 1) {
...
function updateCounterFor(columnSectionEl) {
    ...
    if (counter)
        counter.textContent = columnSectionEl.querySelectorAll('.card').length;
}
...
function refreshUI() {
    for (const name of columnOrder) {
        ...
        listEl
            ...
            .forEach((card) => refreshArrows(card, name));
    }
    updateAllCounters();
}
            ...
            try {
                ...
                if (target.matches('[data-action="add"]')) {
                    ...
                    const card = createCardDOM(
                        { id, content: 'Nowa karta', color: randColor() },
                        section.dataset.col
                    );
                ...
                if (card) {
                    const isInsideEditable =
                        target.closest('[data-role="content"]') != null;
